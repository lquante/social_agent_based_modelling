...
#defines the agent (holds different attributes)
...
population = 100
#set space on which the population lives
space = Agents.GridSpace((10, 10); periodic = false, metric = :euclidean)

#set the external factors (decision landscape)
...
#set up agent based model
    ...
    #populate the grid with diverse agents
    for i = 1:numagents
...
#compute yearly vehicle costs for decision input
...
#syntax: fuelCostKM is equal to -> if vehicle == 1 model.fuelCostKM else model.powerCostKM (one liner)
...
#compute yearly depication of vehicle -> linear with usage
...
function agent_step!(agent, model)
    ...
    # calculate cost of current car (yearlykm times (fuel cost +maintenant*age)):
    ...
    #cld -> rounding (x/y) to nearest int
    feasibleYears = cld(300000, agent.kilometersPerYear) # rounding up division, every agent has their own km preference
    newCombustionCost = 0
    newElectricCost = 0
