...
ensemble <- read.csv("/home/quante/git/social_agent_based_modelling/data/transmission_ensemble_2021-11-23T14:11:23.571.csv")
...
SIR_count = ensemble  %>% filter(transmissionUndetected==0.5,detectionProbability==0.5) %>% group_by(step, SIR_status) %>%tally()
