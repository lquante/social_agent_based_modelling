...
@everywhere begin
    ...
    graph_space = Agents.GraphSpace((3, 3))
...
# set parameters to be varied in the ensemble
    parameters = Dict(
        :space => graph_space,
        #:switchingLimit => [], # assuming that 1 percent of population can be vaccinated per timestep
        ...
        :seed => 1234, # fixed seed to to enough variation by network composition
    )
...
using Dates
date = Dates.now()
identifier = "test"*string(date)*".csv"
